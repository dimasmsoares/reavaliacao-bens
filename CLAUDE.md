...
- `servidor/avaliar.html`: tela principal — sidebar com filtro de texto + toggle "Só pendentes", seletor de metodologia (radio M1/M2/M3), seção de preços (`#section-prices`), seção IPCA (`#section-ipca`), seção de prints (`#section-screenshots`, oculta em M3), campo de valor de mercado editável, observação, botão "Refazer" que abre modal — se `group_reviewed_count > 1` oferece opção de desfazer só este bem ou todos os similares do grupo.
